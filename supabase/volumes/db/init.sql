...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'user_role') THEN
        CREATE TYPE public.user_role AS ENUM ('admin', 'caixa', 'garcom', 'customer');
    END IF;
END$$;
...
CREATE POLICY "Staff can view all orders" ON public.orders FOR SELECT USING ( (SELECT role FROM public.profiles WHERE id = auth.uid()) IN ('admin'::public.user_role, 'caixa'::public.user_role) );
CREATE POLICY "Staff can update orders" ON public.orders FOR UPDATE USING ( (SELECT role FROM public.profiles WHERE id = auth.uid()) IN ('admin'::public.user_role, 'caixa'::public.user_role) );
...
CREATE POLICY "Staff can view all order items" ON public.order_items FOR SELECT USING ( (SELECT role FROM public.profiles WHERE id = auth.uid()) IN ('admin'::public.user_role, 'caixa'::public.user_role) );
